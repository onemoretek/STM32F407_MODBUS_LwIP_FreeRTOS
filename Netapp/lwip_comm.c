...
{ 	
	mem_free(SRAMIN,memp_memory);
	mem_free(SRAMIN,ram_heap);
	mem_free(SRAMIN,TCPIP_THREAD_TASK_STK);
	mem_free(SRAMIN,LWIP_DHCP_TASK_STK);
}
#endif
//lwip 默认IP设置
//lwipx:lwip控制结构体指针
...
{
	u32 sn0;
	sn0=*(unsigned char*)(0x1FFF7A10);//获取STM32的唯一ID的前24位作为MAC地址后三字节
...
void lwip_dhcp_task(void *pdata)
{
	...
	{ 
		...
		if(ip!=0)   					//当正确读取到IP地址的时候
		{
			...
			break;
		}else if(1/*lwip_netif.dhcp->tries>LWIP_MAX_DHCP_TRIES*/) //通过DHCP服务获取IP地址失败,且超过最大尝试次数
		{  
