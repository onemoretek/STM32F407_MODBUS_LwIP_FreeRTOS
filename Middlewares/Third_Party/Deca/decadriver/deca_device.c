...
int dwt_initialise(uint16 config)
{
    ...
    dw1000local.lotID = _dwt_otpread(LOTID_ADDRESS);
		printf("dw1000local.partID = 0x%x\r\n",dw1000local.partID);
		printf("dw1000local.lotID = 0x%x\r\n",dw1000local.lotID);
    ...
    dw1000local.sysCFGreg = dwt_read32bitreg(SYS_CFG_ID) ; // Read sysconfig register
	  printf("dw1000local.sysCFGreg = 0x%x\r\n",dw1000local.sysCFGreg);
    return DWT_SUCCESS ;
...
)
{
    ...
    return readfromspi_serial(cnt, header, length, buffer);  // result is stored in the buffer
